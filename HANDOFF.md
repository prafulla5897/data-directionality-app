...
| 4 — Dashboard | ✅ Complete | 2026-05-07 | tsc clean, build passes, lint clean, 47 unit tests pass; Step6Progress (progress bar + cancel), Step7Results (health snapshot, anomaly cards grouped by display grain, severity filter, methodology accordion, markdown export), Step8Drilldown (comparison bar chart + dual-axis time-series with relative view toggle, dimension context, drill navigation) all implemented; App.tsx wires chunked async analysis execution with progress tracking. Post-phase fixes: B1 week label, B2 scatter → comparison chart, B3 dual Y-axis impressions line, B4 insight direction copy, C1 preset changes, C2 week start day (all 7 weekdays), C3 campaign header, N1 PDF export replacing .md. UX fixes: scatter removed in favour of grouped bar chart showing baseline avg vs anomaly avg with % labels; time-series visibility fixed (solid amber + dashed cyan, 2.5px lines); "Relative view" toggle added (indexed axis, baseline = 100). Bug fixes (post-UX review): black screen on "See detail" fixed (BarController + BarElement were missing from Chart.js registrations in main.tsx); low-magnitude elasticity anomalies suppressed (MIN_ANOMALY_PCT_CHANGE = 0.05 — both metrics must have at least one ≥5% change for elasticity check to fire); ComparisonChart periodEnd edge case fixed. Insight quality fixes: anomaly card S2 now type-aware (elasticity anomalies show historical ratio "1% change in X → Y% in Z"; direction anomalies keep co-movement %); severity badge gains hover tooltip explaining INFO/WARNING/CRITICAL. Drilldown overhaul: elasticity S2 now shows expected vs actual ("a 2% rise in spend typically leads to ~3% in impressions — actual was 15%"); Step 8 summary uses anomaly.body directly (card and drilldown always in sync); time series moved to first position; ComparisonChart replaced with stat table (Metric / Baseline avg / Anomaly avg / Change) — no scale-mismatch; relative view explanation expanded with worked example and formula |
